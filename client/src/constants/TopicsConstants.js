
export const PENDING_TOPICS_PAGE_LENGTH = 12;
export const PENDING_TOPICS_PAGINATION_DELTA = 2;

export const MESSAGES = {
  GET_ALL_FAILED: 'Get all topics failed!',
  ADD_FAILED: 'Failed to add topic!',
  ADD_SUCCESS: 'Successfully added topic!',
};


export const TOPICS_ACTIONS = {
  GET_ALL: 'GET_ALL',
  GET_ALL_FULFILLED: 'GET_ALL_TOPICS_FULFILLED',
  GET_ALL_REJECTED: 'GET_ALL_TOPICS_REJECTED',

  ADD: 'ADD_TOPIC',
  ADD_FULFILLED: 'ADD_TOPIC_FULFILLED',
  ADD_REJECTED: 'ADD_TOPIC_REJECTED',

  SELECT: 'SELECT_TOPIC',
  SELECT_FULFILLED: 'SELECT_TOPIC_FULFILLED',
  SELECT_REJECTED: 'SELECT_TOPIC_REJECTED',
  GET_TOPIC_REQUESTS: 'GET_TOPIC_REQUESTS',
  GET_TOPIC_REQUESTS_FULFILLED: 'GET_TOPIC_REQUESTS_FULFILLED',
  GET_TOPIC_REQUESTS_REJECTED: 'GET_TOPIC_REQUESTS_REJECTED',
  CONFIRM_ACTION: 'CONFIRM_REQUEST_ACTION_APPROVAL',
  CONFIRM_ACTION_SUCCESS: 'CONFIRM_REQUEST_ACTION_APPROVAL_SUCCESS',
  CONFIRM_ACTION_FAILURE: 'CONFIRM_REQUEST_ACTION_APPROVAL_FAILURE',
};

export const URLS = {
  GET_ALL: '/api/topics',
  GET_REQUESTS: '/api/requests',
  APPROVE_REQUEST: '/api/requests',
  ADD: '/api/topics',
};

